    ...
    <div class="my-3 w-1/4">
        <form action="" method="get">
            ...
            <div class="flex items-center mt-2 mb-6">
                ...
                <input type='text' placeholder="Search Blog Title..." name="keyword" value=""
    ...
    <table class="min-w-full bg-white">
        <thead class="bg-gray-800 text-white">
            <tr>
                ...
                <th class="w-1/4"><a href="{{ route('user.create') }}"
        ...
        <tbody class="text-gray-700">
            @foreach ($users as $user)
                <tr>
                    <td class="w-1/12 text-left py-3 px-4">{{ $user->id }}</td>
                    <td class="w-1/4 text-left py-3 px-4">{{ $user->name }}</td>
                    <td class="w-1/4 text-left py-3 px-4">
                        <img class="w-1/2" src="{{ asset('storage/images/user/' . $user->image) }}" alt="image">
                    </td>
                    <td class="w-1/4 text-left py-3 px-4">{{ $user->email }}</td>
                    <td class="w-1/4 text-left py-3 px-4">{{ $user->introduce }}</td>
                    <td class="w-1/4 text-center py-3 px-4">
                        ...
                        <a class="bg-red-600 px-2 py-1 text-white" href="#"
                            onclick="event.preventDefault();document.getElementById('destroyform{{ $user->id }}').submit()">{{ __('DELETE') }}</a>
                        <form style="display: none" action="{{ route('user.destroy', $user->id) }}"
                            id="destroyform{{ $user->id }}" method="post">
                            @method('DELETE')
                            @csrf
                        </form>
                    </td>
