...
@section('content')
    <div class="grid  place-items-center">
        <div class="w-11/12 p-12 bg-white sm:w-11/12 md:w-11/12 lg:w-11/12">
            <h1 class="text-xl font-semibold">Sửa blog </h1>
            <form class="mt-6" method="POST" action="{{ route('bai-viet.update', ['id' => $blog->id]) }}"
                enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                <label class="text-gray-600 font-light" for="name">Tiêu đề bài viết</label>
                <input type='text' placeholder="Enter your input here" name="title" value="{{ $blog->title }}"
                ...
                <select name="cate_id"
                    ...
                    @foreach ($cates as $cate)
                        <option @if ($cate->id == $blog->cate->id) selected @endif value="{{ $cate->id }} ">{{ $cate->name }}</option>
                    @endforeach
                </select>
                <label class="block" for="desc">
                ...
                <textarea name="short_desc"
                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-">{{ $blog->short_desc }}</textarea>
                <label class="block" for="desc">
                    @error('short_desc')
                        <div class="text-red-500 text-sm mb-2">
                            {{ $message }}
                ...
                <textarea name="content" class="form-control my-editor">{{ $blog->content }}</textarea>
                @error('content')
                    <div class="text-red-500 text-sm mb-2">
                        {{ $message }}
                    </div>
